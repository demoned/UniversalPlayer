...
object Utils {
    ...
    fun saveProgress(context: Context, url: Any?, progress: Long) {
        ...
        if (!BasePlayerView.Companion.SAVE_PROGRESS) return
        if (progress < 5000) {
            progress = 0
        }
    ...
    fun getSavedProgress(context: Context, url: Any?): Long {
        if (!BasePlayerView.Companion.SAVE_PROGRESS) return 0
        val spn = context.getSharedPreferences(
            "progress",
            Context.MODE_PRIVATE
        )
    ...
    fun showStatusBar(context: Context?) {
        if (BasePlayerView.Companion.TOOL_BAR_EXIST) {
    ...
    fun showSystemUI(context: Context?) {
        ...
        getWindow(context).decorView.systemUiVisibility =
            SYSTEM_UI
    }
    ...
    fun getNavigationBarHeight(context: Context): Int {
        val var1 = ViewConfiguration.get(context).hasPermanentMenuKey()
        var var2: Int
